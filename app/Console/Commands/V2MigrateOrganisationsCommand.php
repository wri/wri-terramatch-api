...
class V2MigrateOrganisationsCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'v2-migrate-organisations';
     ...
     * @var string
     */
    protected $description = 'Update legacy organisations to V2';

    public function handle()
    {
        $collection = Organisation::all();
        foreach ($collection as $organisation) {
            $version = $organisation->approved_version;
            if (! empty($version)) {
                ...
                $organisation->status = $organisation->status ?? V2Organisation::STATUS_APPROVED;
