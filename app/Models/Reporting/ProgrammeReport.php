...
class ProgrammeReport extends CustomReport implements CustomReportInterface
{
    ...
    {
        $export = [];
        $programme = $this->exportable;
        $submissions = $this->date_from ? $programme->submissions()->where('created_at', '>', $this->date_from)->get() : $programme->submissions;
    ...
    private function addAdditionalSpeciesFiles(Submission $submission): void
    {
        foreach (data_get($submission, 'documentFiles', []) as $documentFiles) {
            if ($documentFiles->collection == 'tree_species') {
                $name = $documentFiles->title ?? Str::uuid();
