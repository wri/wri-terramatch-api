...
class DocumentationVersionedController extends Controller
{
    public function readAsHtmlAction(Request $request, string $version = 'v1', string $ui = 'redoc'): View
    {
        $this->authorize('yes', 'App\\Models\\Default');

        if (strtolower($ui) === 'swagger') {
            return ViewFactory::make('Documentation/readAsHtmlSwagger', compact('version'));
        }

        return ViewFactory::make('Documentation/readAsHtmlRedoc', compact('version'));
    }
     ...
     */
    public function readAsYamlAction(Request $request, string $version = 'v1'): Response
    {
        $this->authorize('yes', 'App\\Models\\Default');
        $url = config('app.url');
        ...
        $yaml = str_replace(
            ...
            file_get_contents(__DIR__ . '/../../../resources/docs/swagger-' . $version . '.yml')
        );
