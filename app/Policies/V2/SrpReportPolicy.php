...
class SrpReportPolicy extends Policy
{
    public function read(?User $user, ?SrpReport $report = null): bool
    {
    ...
    public function readAll(?User $user, ?SrpReport $report = null): bool
    {
        return $user->hasAnyPermission(['projects-manage', 'framework-terrafund', 'framework-ppc', 'framework-hbf']);
    }

    public function update(?User $user, ?SrpReport $report = null): bool
    {
        if ($user->can('framework-' . $report->framework_key)) {
            return true;
        }
    ...
    public function submit(?User $user, ?SrpReport $report = null): bool
    {
        if ($user->can('projects-manage') && $this->isManaging($user, $report)) {
            return true;
        }
    ...
    public function delete(?User $user, ?SrpReport $report = null): bool
    {
        if ($user->can('projects-manage') && $this->isManaging($user, $report)) {
            return true;
        }
    ...
    public function uploadFiles(?User $user, ?SrpReport $report = null): bool
    {
        if ($user->can('projects-manage') && $this->isManaging($user, $report)) {
            return true;
        }
    ...
    public function approve(?User $user, ?SrpReport $report = null): bool
    {
        if ($user->can('projects-manage') && $this->isManaging($user, $report)) {
            return true;
        }
    ...
    public function updateFileProperties(?User $user, ?SrpReport $report = null): bool
    {
        if ($user->can('framework-' . $report->framework_key)) {
            return true;
        }
    ...
    public function deleteFiles(?User $user, ?SrpReport $report = null): bool
    {
        if ($user->can('framework-' . $report->framework_key)) {
            return true;
        }
    ...
    protected function isTheirs(?User $user, ?SrpReport $report = null): bool
    {
        return $user->organisation->id == $report->project->organisation_id || $user->projects->contains($report->project->id);
    }

    protected function isManaging(?User $user, ?SrpReport $report = null): bool
    {
