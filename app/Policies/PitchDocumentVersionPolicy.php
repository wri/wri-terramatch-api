...
{
    public function read(?User $user, ?PitchDocumentVersion $model = null): bool
    {
        return $this->isVerifiedAdmin($user) || ($this->isFullUser($user) && $this->isOwner($user, $model));
    }

    public function approve(?User $user, ?PitchDocumentVersion $model = null): bool
    {
        $isPending = $model->status == "pending";
        return $this->isVerifiedAdmin($user) && $isPending && !$this->isCompleted($user, $model);
    }

    public function reject(?User $user, ?PitchDocumentVersion $model = null): bool
    {
        $isPending = $model->status == "pending";
        return $this->isVerifiedAdmin($user) && $isPending && !$this->isCompleted($user, $model);
    }

    public function delete(?User $user, ?PitchDocumentVersion $model = null): bool
    {
        $isPending = $model->status == "pending";
