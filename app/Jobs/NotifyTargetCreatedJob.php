...
class NotifyTargetCreatedJob implements ShouldQueue
{
    ...
    public function handle()
    {
        $organisationId =
            $this->model == "Offer" ?
            $this->target->monitoring->match->interest->offer->organisation_id :
            $this->target->monitoring->match->interest->pitch->organisation_id;
        $targetId = $this->target->id;
        $name = $this->target->monitoring->match->interest->pitch->approved_version->name;
    ...
    private function notifyUsers(Int $organisationId, Int $targetId, String $name)
    {
        ...
        foreach ($users as $user) {
            if ($user->is_subscribed) {
                Mail::to($user->email_address)->send(new TargetCreatedMail($targetId, $name));
            }
            /*
