...
class NotifyProjectUpdatedJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        switch ($model) {
            case "Offer":
                $matches = InterestModel::where("matched", "=", 1)
                    ->where("initiator", "=", "pitch")
                    ->where("offer_id", "=", $id)
                    ->get();
                $interests = InterestModel::where("matched", "=", 0)
                    ->where("initiator", "=", "pitch")
                    ->where("offer_id", "=", $id)
                    ->get();
                break;
        ...
        $organisationIds = [
            "matched" => arr_uv($matches->pluck("organisation_id")->toArray()),
            "interested" => arr_dv(
        ...
        $this->notifyMatchedUsers($organisationIds["matched"], $model, $id);
    ...
    private function notifyMatchedUsers(Array $organisationIds, String $model, Int $id)
    {
        ...
        foreach ($users as $user) {
            if ($user->is_subscribed) {
                Mail::to($user->email_address)->send(new ProjectUpdatedMail("Match", $model, $id));
            }
