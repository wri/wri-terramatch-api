...
class PublishDraftOfferJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            foreach ($dataArray["offer_contacts"] as $offerContactsDatum) {
                ...
                ControllerHelper::callAction("OfferContactsController@createAction", $offerContactsDatum);
            }
            foreach ($dataArray["offer_documents"] as $offerDocumentsDatum) {
                $offerDocumentsDatum["offer_id"] = $offer->data->id;
                ControllerHelper::callAction("OfferDocumentsController@createAction", $offerDocumentsDatum);
            }
            $this->draft->delete();
            DB::commit();
            Cache::forget($key);
